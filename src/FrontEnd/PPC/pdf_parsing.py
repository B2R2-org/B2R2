    ...
    for offset in offsets_line.split(" "):
        ...
        for chr in offset:
            ...
            if int(offset_curr + chr) >= 32:
                ...
                if len(offsets) > 0 and offsets[-1] >= int(offset_curr):
                    raise ParsingError
...
def to_json(operands_line, fields_line, offsets_line):
    ...
    if len(offsets) != len(fields) + 1 or len(operands) == 0:
        raise ParsingError
    ...
    for i in range(len(fields)):
        ...
        if field.isdecimal():
            if (1 << (field_range[1] - field_range[0] + 1)) - 1 < int(field):
                raise ParsingError
...
def extract_instrs(page, page_num):
    lines = page.extract_text(x_tolerance = 0.3).split("\n")
    instrs = []
    for i in range(2, len(lines)):
        ...
        for operands_line in lines[start_idx:end_idx]:
            try:
                ...
                print(f"Parsing error occurred on page {page_num}.")
...
for i in range(start_page, end_page + 1):
    ...
    left_instrs = extract_instrs(left_page, i)
    right_instrs = extract_instrs(right_page, i)

    all_instrs.extend(left_instrs)
    all_instrs.extend(right_instrs)

    if len(left_instrs) > 0 or len(right_instrs) > 0:
